...
class ASRConfig:
    ...
    # Error handling
    # If True, return None on persistent errors instead of raising
...
class ASRService:
    ...
    def _load_model_if_possible(self) -> None:
        ...
        try:
            ...
            device_for_model = self._device if self._device is not None else "cpu"
            self._model = self._model.to(device_for_model)
            self._model.eval()
    ...
    def _load_audio(
        ...
        if torchaudio is not None:
            try:
                ...
                # torchaudio failed (e.g., TorchCodec not available),
                # fall back to wave module
                if self.cfg.log_errors:
                    error_msg = str(e)
                    # Only log if it's not the common TorchCodec error
                    # (to reduce spam)
                    if "TorchCodec" not in error_msg:
                        print(
                            f"***ASR: torchaudio failed for {audio_path}, "
                            f"falling back to wave module: {e}"
                        )
    ...
    def _transcribe_audio(
        ...
        Args:
            audio_array: Audio samples as numpy array
                (float32, mono, normalized to [-1, 1])
        ...
        try:
            ...
            if self.cfg.log_errors:
                print(
                    ...
                    f"({duration:.2f}s) in {processing_time:.2f}s"
                )
