...
def measure_inference(
    ...
    if results['audio_duration_sec']:
        rtf = avg_inference_time / results['audio_duration_sec']
        results['rtf'] = rtf
...
def print_results(results: dict):
    ...
    if 'rtf' in results:
        ...
        if results['rtf'] < 1.0:
            print(f"  ✅ Faster than real-time (good for live transcription)")
        elif results['rtf'] < 2.0:
            print(f"  ⚠️  Slower than real-time (may struggle with live)")
        else:
            print(f"  ❌ Much slower than real-time (batch only)")
